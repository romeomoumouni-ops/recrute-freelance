...
import { isAbonnementSale, creditAbonnement } from '@/lib/chariow-abonnement';
...
async function handleSale(payload: any) {
  ...
  // Abonnement freelance ? (au cas où la vente d'abonnement arrive aussi sur ce webhook)
  if (isAbonnementSale(payload)) {
    await creditAbonnement(saleId, email, payload);
    return;
  }
